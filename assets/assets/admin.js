(() => {
  ...
  function ensureStockSyncMeta(){
    if(!state.doc || typeof state.doc !== "object") state.doc = { categories: [], products: [], popups: [] };
    if(!state.doc._meta || typeof state.doc._meta !== "object") state.doc._meta = {};
    if(!state.doc._meta.stockSync || typeof state.doc._meta.stockSync !== "object") state.doc._meta.stockSync = {};
    const sync = state.doc._meta.stockSync;
    if(!sync.baselineByProduct || typeof sync.baselineByProduct !== "object" || Array.isArray(sync.baselineByProduct)){
      sync.baselineByProduct = {};
    }
    return sync;
  }

  function refreshStockBaselineForProducts(productIds){
    const ids = [...new Set((Array.isArray(productIds) ? productIds : []).map(id => String(id || "")).filter(Boolean))];
    if(!ids.length) return false;

    const sync = ensureStockSyncMeta();
    const sold = getSoldQtyByProduct();
    let changed = false;

    for(const pid of ids){
      const p = prodById(pid);
      if(!p) continue;
      const nextBase = Math.max(0, Number(p.stock || 0) + Number(sold.get(pid) || 0));
      if(Number(sync.baselineByProduct[pid] || 0) !== nextBase){
        sync.baselineByProduct[pid] = nextBase;
        changed = true;
      }
    }
  ...
  function syncStocksFromSales(options = {}){
    const opts = options || {};
    const sync = ensureStockSyncMeta();
    const sold = getSoldQtyByProduct();
    const liveIds = new Set((state.doc.products || []).map(p => String(p?.id || "")).filter(Boolean));
    ...
    for(const p of (state.doc.products || [])){
      ...
      const soldQty = Number(sold.get(pid) || 0);
      if(!Number.isFinite(Number(sync.baselineByProduct[pid]))){
        sync.baselineByProduct[pid] = Math.max(0, Number(p.stock || 0) + soldQty);
        metaChanged = true;
      }

      if(p.status === "soon") continue;

      const nextStock = Math.max(0, Number(sync.baselineByProduct[pid] || 0) - soldQty);
      const currentStock = Math.max(0, Number(p.stock || 0));
      if(currentStock !== nextStock){
        p.stock = setProductStockValue(pid, nextStock, { absolute:true });
        stockChanged = true;
      }

      const nextStatus = nextStock <= 0 ? "out" : "ok";
    ...
    Object.keys(sync.baselineByProduct || {}).forEach(pid => {
      if(!liveIds.has(String(pid))){
        delete sync.baselineByProduct[pid];
        metaChanged = true;
      }
    });

    if(!(stockChanged || metaChanged)) return false;
    ...
    if(!opts.silent){
      ...
      try{ if($("#panelChart") && $("#panelChart").style.display !== "none") drawChart(); }catch{}
    }

    markDirty({ products:true, stockOnly: !metaChanged, fast:true });
    return true;
  }

  let _stockSyncTick = null;
  function startStockSyncTicker(){
    try{ if(_stockSyncTick) clearInterval(_stockSyncTick); }catch{}
    _stockSyncTick = setInterval(() => {
      try{
        if(!state.loaded || state.saving || state.dirty) return;
        syncStocksFromSales({ silent:true });
      }catch(err){
        console.error("Stock sync timer error:", err);
      }
    }, 5 * 60 * 1000);
  }
  ...
  async function buildProductsSavePlan(cfg){
    ...
    if(stockOnly){
      ...
      docToSave._meta = {
        ...(docToSave._meta || {}),
        ...JSON.parse(JSON.stringify((state.doc && state.doc._meta) ? state.doc._meta : {}))
      };
    }
    ...
    $("#panelProducts").querySelectorAll("[data-pid]").forEach(el => {
      const apply = () => {
        ...
        if(k === "stock"){
          ...
          refreshStockBaselineForProducts([pid]);
        ...
        if(k === "status") refreshStockBaselineForProducts([pid]);
  ...
  function openProductModal(id){
    ...
    openModal(editing ? "Termék szerkesztése" : "Új termék", "", body, [
      ...
      { label:"Mentés", kind:"primary", onClick: () => {
        ...
        refreshStockBaselineForProducts([np.id]);
        closeModal();
        renderAll();
        markDirty({ products:true });
      }}
...
function deleteSale(id){
  ...
  const rangeDays = (startDate, endDate) => {
    ...
    const endTs = new Date(endDate).setHours(0,0,0,0);
    return Math.max(1, Math.round((endTs - startTs) / 86400000) + 1);
  };

  const buildAdaptiveSeries = (startDate, endDate, sourceMap, dailyLimit = 62) => {
    return rangeDays(startDate, endDate) <= dailyLimit
      ? buildDailySeries(startDate, endDate, sourceMap)
      : buildMonthlySeries(startDate, endDate, sourceMap);
  };
  ...
  const drawLine = (canvas, points) => {
    ...
    const labelIndexes = [];
    const maxLabels = Math.max(2, Math.min(n, Math.floor(pw / 78) + 1));
    const step = Math.max(1, Math.ceil((n - 1) / Math.max(1, maxLabels - 1)));
    for(let i = 0; i < n; i += step) labelIndexes.push(i);
    if(labelIndexes[labelIndexes.length - 1] !== n - 1) labelIndexes.push(n - 1);

    [...new Set(labelIndexes)].forEach((idx) => {
      const point = safePoints[idx];
      if(!point) return;
      ctx.fillText(formatAxisLabel(point.label), xAt(idx), padT + ph + 10);
    });
  };

  const seriesAll = buildAdaptiveSeries(allFrom, allTo, allDayMap);
  const seriesYear = buildAdaptiveSeries(yearStart, yearEnd, yearDayMap);
